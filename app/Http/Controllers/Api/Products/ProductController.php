...
class ProductController extends MasterApiController
{
    ...
    public function __invoke(Request $request)
    {
        ...
        $products = $productQuery->with('brand')->withAvg('rating','rate')->get();
    ...
    private function checkAuthUser($queryData,$request)
    {
        if ($request->bearerToken()){
            ...
            $cartIds = $user ?  $user['orders']?->pluck('orderItems')?->flatten()?->pluck('product_id')?->toArray() : [];
            $favoriteIds = $user ?  $user['favorite_products']?->pluck('id')?->toArray() : [];
