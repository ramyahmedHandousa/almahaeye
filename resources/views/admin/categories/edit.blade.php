...
@section('content')
    ...
    <form method="POST" action="{{ route('categories.update', $category->id) }}@if(request('type'))?type={{request('type')}} @endif"
        ...
        <div class="row">
            ...
            <div class="col-sm-8 " >
                <div class="card-box">
                    ...
                    <h4 class="header-title m-t-0 m-b-30">تعديل بيانات  القسم</h4>

                    @if(request('type'))
                        <input type="hidden" name="type" value="sub">
                        <div class="col-xs-12">
                            <div class="form-group">
                                <label for="userName">إسم القسم*</label>
                                <select name="parent_id"  class="form-control requiredFieldWithMaxLenght" required>
                                    <option value="" selected disabled="">إختر القسم</option>
                                    @foreach($masterCategories as $value)
                                        <option value="{{ $value->id }}" @if($value->id == $category->parent_id) selected @endif>
                                            {{ $value->name }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="col-xs-12">
                        <div class="form-group{{ $errors->has('name_ar') ? ' has-error' : '' }}">
                            ...
                            <input type="text" name="name_ar" value="{{ $category->name }}"
                                   class="form-control"
                    ...
                    <div class="col-xs-12">
                        <div class="form-group{{ $errors->has('name_en') ? ' has-error' : '' }}">
                            ...
                            <input type="text" name="name_en" value="{{ $category?->translate('en')?->name }}"
                                   class="form-control"
                    ...
                    <div class="form-group">
                        <div class="col-sm-12">
                            <input type="file" name="image" class="dropify" data-max-file-size="6M"
                                   accept='.jpeg,.png,.jpg'
                                   data-default-file="{{$category->getFirstMediaUrl('master_image','thumb')}}"
                            />
                        </div>
                    </div>
                </div>
